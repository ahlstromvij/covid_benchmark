...
texreg::texreg(list(m_govt_perf_int_cont, # contact,
               ...
               caption.above = TRUE,
               override.se = list(coeftest(m_govt_perf_int_cont, vcov = m_govt_perf_int_cont_vcov)[,2],
                                  coeftest(m_level_concern_int, vcov = m_level_concern_int_vcov)[,2],
                                  coeftest(m_restrict_int_contact, vcov = m_restrict_int_contact_vcov)[,2]),
               override.pvalues = list(coeftest(m_govt_perf_int_cont, vcov = m_govt_perf_int_cont_vcov)[,4],
                                       coeftest(m_level_concern_int, vcov = m_level_concern_int_vcov)[,4],
               ...
               "  x Contact"),
               reorder.coef = c(1, 2, 22, 3, 23, 4:10, 12, 11, 13, 14, 18, 17, 16, 15, 19, 20, 21),
               custom.model.names=c("Gov. support", "Concern",
                            "Supp. restrictions"),
               float.pos = "h!",
               symbol = "\\cdot",
               stars = c(0.001, 0.01, 0.05, 0.1),
...
texreg::texreg(list(m_govt_perf_int_know, # knowledge
               ...
               custom.model.names=c("Gov't perf.", "Activity",
                            "Supp. restrictions"),
              override.se = list(coeftest(m_govt_perf_int_know, vcov = m_govt_perf_int_know_vcov)[,2],
                                 coeftest(m_behavior_int, vcov = m_behavior_int_vcov)[,2],
                                 coeftest(m_restrict_int_know, vcov = m_restrict_int_know_vcov)[,2]),
               override.pvalues = list(coeftest(m_govt_perf_int_know, vcov = m_govt_perf_int_know_vcov)[,4],
                                       coeftest(m_behavior_int, vcov = m_behavior_int_vcov)[,4],
